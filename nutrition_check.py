...
def show():
    ...
    st.subheader("หมายเหตุ: โปรแกรมนี้ไม่สามารถตรวจสอบคำกล่าวอ้างเชิงเปรียบเทียบได้")
    ...
    not_in_list_option = "ไม่อยู่ในบัญชีหมายเลข 2"
    group_names = [name for name in food_groups["group_name"].dropna().unique().tolist() if str(name).strip()]
    group_options = [not_in_list_option] + group_names
    selected_group = st.selectbox("เลือกกลุ่มอาหารตามบัญชีหมายเลข 2 (พิมพ์เพื่อค้นหาได้เลย)", group_options)

    selected_label = not_in_list_option

    if selected_group != not_in_list_option:
        group_foods = food_groups[food_groups["group_name"] == selected_group]
        food_type_options = group_foods["food_type_th"].dropna().tolist()
        if not food_type_options:
            st.error("ไม่พบชนิดอาหารในกลุ่มที่เลือก")
            st.stop()
        selected_label = st.selectbox("เลือกชนิดอาหารในกลุ่มที่เลือก (พิมพ์เพื่อค้นหาได้เลย)", food_type_options)

        group_info = group_foods[group_foods["food_type_th"] == selected_label].iloc[0].copy()  # Copy to allow modification
        ...
        if missing_serving:
            st.warning("⚠️ ไม่พบข้อมูลหน่วยบริโภคอ้างอิง กรุณากรอกวิธีเตรียมให้ครบทั้ง 3 ช่อง (กรณีเป็นนม/เครื่องดื่ม กรอกหน่วยบริโภคอ้างอิง 200 มล. กรณีซุปผงกรอก 150 มล.)")
            col_powder, col_water, col_ref = st.columns(3)
            with col_powder:
                powder_once = float_input("ปริมาณผงต่อการบริโภค 1 ครั้ง (กรัม)")
            with col_water:
                water_used = float_input("ปริมาณน้ำที่ใช้ละลาย (มิลลิลิตร)")
            with col_ref:
                reference_unit_ml = float_input("หน่วยบริโภคอ้างอิงของอาหาร (มิลลิลิตร)")
            if any(x is None for x in (powder_once, water_used, reference_unit_ml)):
                st.error("กรุณากรอกให้ครบทั้ง 3 ช่องเพื่อดำเนินการต่อ")
                st.stop()
            if powder_once <= 0 or water_used <= 0 or reference_unit_ml <= 0:
                st.error("ค่าที่กรอกต้องมากกว่า 0")
                st.stop()
            manual_serving = (powder_once / water_used) * reference_unit_ml
            group_info["serving_value"] = manual_serving
            group_info["unit"] = "กรัม"
            is_ref_serving_user_input = True
            st.info(f"กลุ่มนี้ใช้หน่วยบริโภคอ้างอิง (คำนวณ): {manual_serving:.2f} กรัม")
        else:
    ...
    nutrition_check_method = st.radio(
        "วิธีการตรวจสอบ:(**⚠️แนะนำให้ใช้การตรวจสอบจากผลวิเคราะห์จะมีความแม่นยำกว่า**)",
    ...
    if st.button("🔍 ตรวจสอบคำกล่าวอ้าง"):
                    ...
                    if nutrition_check_method == "ตรวจสอบจากผลวิเคราะห์โภชนาการ (ต่อ 100 g หรือ ml)":
                        ...
                        if ref_value <= 30:
                            if is_ref_serving_user_input:
                                st.info(f"สำหรับหน่วยบริโภคอ้างอิงที่ผู้ใช้กำหนด ≤ 30 {ref_unit}: ระบบจะตรวจสอบเงื่อนไขการกล่าวอ้างจากปริมาณ 2 เท่าของหน่วยบริโภคอ้างอิงและปริมาณที่กินต่อครั้งที่แสดงบนฉลาก โดยในกรณีนี้จะปรับจากค่าหน่วยบริโภคอ้างอิงจากค่าต่อ 100 {ref_unit} (ตามผลวิเคราะห์) เป็นค่าต่อ {ref_value} {ref_unit}")
                            else:
                                st.info(f"สำหรับอาหารที่มีหน่วยบริโภคอ้างอิง ≤ 30 {ref_unit}: ระบบจะตรวจสอบเงื่อนไขการกล่าวอ้างจากปริมาณ 2 เท่าของหน่วยบริโภคอ้างอิงและปริมาณที่กินต่อครั้งที่แสดงบนฉลาก โดยในกรณีนี้จะปรับจากค่าหน่วยบริโภคอ้างอิงจากค่าต่อ 100 {ref_unit} (ตามผลวิเคราะห์) เป็นค่าต่อ {ref_value * 2} {ref_unit}")
                        else:
                            st.info(f"สำหรับอาหารที่มีหน่วยบริโภคอ้างอิง > 30 {ref_unit}: ระบบจะตรวจสอบเงื่อนไขการกล่าวอ้างจากปริมาณหน่วยบริโภคอ้างอิงและปริมาณที่กินต่อครั้งที่แสดงบนฉลาก โดยในกรณีนี้จะปรับจากค่าหน่วยบริโภคอ้างอิงจากค่าต่อ 100 {ref_unit} (ตามผลวิเคราะห์) เป็นค่าต่อ {ref_value} {ref_unit}")
                    else:
            ...
            if selected_label != "ไม่อยู่ในบัญชีหมายเลข 2":
                if group_info is not None and isinstance(group_info, pd.Series) and 'serving_value' in group_info:
                    ...
                    if ref_value <= 30:
                        ...
                        st.info("กรณีอาหารที่มีหน่วยบริโภคอ้างอิง > 30 กรัม: ระบบจะตรวจสอบเงื่อนไขการกล่าวอ้างจากปริมาณหน่วยบริโภคอ้างอิงและปริมาณที่กินต่อครั้งที่แสดงบนฉลาก")
                else: 
        ...
        # ตรวจสอบทั่วไป
        should_show_rule11 = False  # flag to show rule 1.1 once when any claim has condition==2 (table1)
        for idx, row in claims.iterrows():
            ...
            # แสดงคำเตือนเงื่อนไขข้อ 1.1 ให้อยู่เหนือข้อความภาษาไทย ถ้ามีข้อกำหนด condition == 2 (Table 1)
            # จะทำการแสดงคำเตือนเงื่อนไขข้อ 1.1 ตอนท้าย ก่อนข้อความภาษาไทย (ย้ายตำแหน่งไปช่วงท้ายแล้ว)
                ...
                if pd.notna(condition_id) and not condition_lookup.empty:
                    try:
                        ...
                        # Mark to show rule 1.1 notice later (once), if any claim requires condition 2 in Table 1
                        if ("2" in condition_ids) and (table_type == "table1"):
                            should_show_rule11 = True
        ...
        if not results_found:
            ...
            # แสดงคำเตือนเงื่อนไขข้อ 1.1 (ถ้าจำเป็น) ให้อยู่เหนือข้อความภาษาไทย
            if should_show_rule11:
                st.warning(
                    "เงื่อนไขข้อ 1.1: ไม่อนุญาตให้กล่าวอ้างว่า \"ปราศจาก\" หรือ \"ต่ำ\" หากอาหารชนิดนั้นในธรรมชาติเป็นไปตามเงื่อนไขอยู่แล้ว โดยไม่ได้ใช้กระบวนการผลิตที่พิเศษ หรือไม่มีการปรับสูตร",
                    icon="⚠️",
                )
            
            # แสดงข้อความเตือน หลังจากแสดงผลการตรวจสอบทั้งหมด
            st.warning("📢 **ข้อความกล่าวอ้างต้องระบุเป็นข้อความภาษาไทย ทั้งนี้ อาจระบุข้อความภาษาอื่นๆเพิ่มเติมได้ โดยต้องสอดคล้องกับภาษาไทย**", icon="⚠️")
            st.warning("📢 แม้ปริมาณสารอาหารจะเป็นไปตามเงื่อนไขของการกล่าวอ้างแต่ต้องตรวจสอบเงื่อนไขเพิ่มเติมอื่นๆให้ครบถ้วนตามที่ประกาศฯ 445 กำหนดไว้", icon="⚠️")
            ...
            if final_disclaimer_results:
                ...
                st.write("ต้องกำกับด้วยข้อความแสดงปริมาณไขมันทั้งหมด หรือไขมันอิ่มตัว หรือคอเลสเตอรอล หรือโซเดียม หรือน้ำตาลทั้งหมด ที่อยู่ในระดับเกินปริมาณที่กำหนดไว้ติดกับข้อความกล่าวอ้างที่มีขนาดใหญ่ที่สุดหรือเห็นได้ชัดที่สุดบนฉลากด้วย โดยข้อความกำกับนี้ต้องมีขนาดไม่เล็กกว่าครึ่งหนึ่งของข้อความกล่าวอ้าง")
                st.write("ตัวอย่าง: **โปรตีนสูง** มีไขมันอิ่มตัว 3 กรัม ต่อ 50 กรัม")
