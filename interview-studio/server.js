...
async function handleChatFollowup(req, res) {
  try {
    ...
    const currentMessages = messages.filter((message) => message.questionId === currentQuestion.id);
    const scopedMessages = currentMessages.length ? currentMessages : messages.slice(-8);
    const latestUserText =
      [...scopedMessages].reverse().find((message) => message.role === "user")?.text || "";
    const normalizedContents = normalizeGeminiContents(scopedMessages);
...
{
  "response": "คำถามเจาะลึก 1 ข้อที่สั้น กระชับ สุภาพ (1-2 ประโยค) หรือข้อความขอบคุณสั้น ๆ เมื่อข้อมูลข้อนี้ครอบคลุมแล้ว",
  "shouldMoveToNext": true หรือ false
}

รายละเอียดหัวข้อปัจจุบัน:
รหัสข้อ: ${currentQuestion.id}
หัวข้อ: ${currentQuestion.title}
ข้อความล่าสุดของผู้ให้สัมภาษณ์: ${latestUserText || "-"}
...
หลักการถามเจาะลึก:
- วิเคราะห์คำตอบล่าสุดก่อนเสมอ แล้วถามต่อจากสิ่งที่ผู้ให้สัมภาษณ์เพิ่งตอบ ไม่ใช้คำถามสำเร็จรูปแบบไม่เกี่ยวกับคำตอบ
- ถามทีละคำถามสั้น ๆ เป็นกันเอง สุภาพ มีหางเสียง (ครับ/ค่ะ)
- ถ้าผู้ให้สัมภาษณ์ถามกลับ เช่น "อะไรคะ", "คำถามคืออะไร", "เล่าอะไร", "ไม่เข้าใจ" หรือพิมพ์แค่ "ค่ะ/ครับ" ให้ทวนคำถามหลักด้วยภาษาง่าย ๆ และกำหนด shouldMoveToNext เป็น false
- ห้ามถามว่า "ช่วยเล่าเป็นเหตุการณ์จริง" แบบทั่วไป ยกเว้นข้อ q2-2 ที่ต้องการตัวอย่างปัญหาจริงและผู้ตอบยังไม่ได้เล่ากรณีใดเลย
- ห้ามสรุปว่าข้อมูลครบถ้วนเร็วเกินไป ถ้ายังไม่ได้แตะประเด็นสำคัญอย่างน้อย 2-3 probe หรือผู้ตอบยังไม่ได้บอกว่าไม่มีเพิ่มเติม
- ถ้า shouldMoveToNext เป็น true ให้ response เป็นเพียงคำขอบคุณ/สรุปสั้น ๆ ของข้อปัจจุบัน ไม่ต้องถามข้อถัดไป เพราะระบบหน้าเว็บจะเป็นผู้เปิดคำถามถัดไปเอง
- ห้ามอ้างประโยคจากข้ออื่นหรือบทสนทนาก่อนหน้าเป็นคำตอบของหัวข้อปัจจุบัน`;
